...
from EDUConnect.models import Post, Comment, User, Announcement
...
def admin_dashboard():
    if current_user.role != "admin":
        ...
        total_announcement = Announcement.query.count()

        return render_template("admin_dashboard.html", total_posts=total_posts, total_users=total_users, total_comments=total_comments, total_announcement=total_announcement)
...
@admin.route('/announcement')
def announcement():
    page = request.args.get('page', 1, type=int)
    announcement = Announcement.query.paginate(page=page, per_page=5)
...
@admin.route('/delete_announcement/<int:id>', methods=['DELETE'])
