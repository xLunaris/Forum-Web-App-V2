...
from EDUConnect.models import Post, Comment, User, Reply
...
# Function to retrieve all replies for a specific comment
@posts.route("/replies/<int:comment_id>", methods=["GET"])
def get_replies(comment_id):
    replies = Reply.query.filter_by(comment_id=comment_id).all()
    return jsonify([reply.serialize() for reply in replies])
